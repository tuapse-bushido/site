 ...
 * - Source: addon_rule_target_product → addon_rule
 * - Filter: only addon rules with is_active = true
 * - Output: product_id and corresponding addon_rule_id
 ...
 * - Источник: addon_rule_target_product → addon_rule
 * - Фильтрация: только активные правила добавок (is_active = true)
 * - Выходные данные: product_id и addon_rule_id
...
FROM addon_rule_target_product artp
         JOIN addon_rule adr ON adr.id = artp.addon_rule_id
